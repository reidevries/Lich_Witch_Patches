...
class CircularBuffer
{
  ...
  public:
    ...
    void write(FloatArray source) { write(source.getData(), source.getSize()); }
    ...
    void write(float* source, size_t len)
    {
        ...
        while (ptr < end && cnt--) *ptr++ = *source++;
        ptr = &buffer[0];
        while (cnt-- > 0) *ptr++ = *source++;
    }
    ...
    {
        if (++writeIndex == getSize()) writeIndex = 0;
        buffer[writeIndex] = value;
    }

    /**
     * read the value @param index steps back from the head of the circular
     * buffer
     */
    ...
    {
        float* ptr =
          &buffer[(writeIndex + ~(readIndex + len)) &
                  (buffer.getSize() - 1)]; // start 'len' samples back
        float* end = &buffer[buffer.getSize()];
        int cnt = len;
        while (ptr < end && cnt--) *destination++ = *ptr++;
        ptr = &buffer[0];
        while (cnt-- > 0) *destination++ = *ptr++;
    }
     ...
     */
    inline unsigned int getSize() { return buffer.getSize(); }
    ...
    void setAll(float value) { buffer.setAll(value); }

    void clear() { setAll(0); }

    FloatArray getSamples() { return buffer; }
