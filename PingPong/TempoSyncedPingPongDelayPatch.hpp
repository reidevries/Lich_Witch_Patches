...
static const uint32_t counters[RATIOS_COUNT] = { 1, 1, 1, 1, 1, 3, 2, 3, 4 };
...
class TempoSyncedPingPongDelayPatch : public Patch
{
  private:
    ...
    DcFilter speed_filter;
    StereoDcFilter dc;
    StereoBiquadFilter* lowpass;
    RampOscillator<RAMP_SHAPE>* lfo1;
    SineOscillator* lfo2;
    SmoothFloat time;
    SmoothFloat drop;
  ...
  public:
	...
	// called when tap tempo button is pressed
    ...
    void processAudio(AudioBuffer& buffer)
    {
        ...
        for (int n = 0; n < size; n++) {
            ...
            float x0 = 1.0 - x1;
            float ldly = delayBufferL->read(delayL) * x0 +
                         delayBufferL->read(newDelayL) * x1;
            float rdly = delayBufferR->read(delayR) * x0 +
                         delayBufferR->read(newDelayR) * x1;
            // ping pong
